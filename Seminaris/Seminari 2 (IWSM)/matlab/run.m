...
ds = readtable('cases_processed.xls');
...
%parpool(16); % for parallel computing (16 cores, change accordingly)
tbl=tabulate(ds{:,4});
for i=1:size(tbl, 1)
    ca = tbl(i)
    y = 
    theta_BSL = bayes_sl_ricker_wood(y,1,500,n,cov_rw);  % NOTE: this will take many hours to run (reduce number of iterations if desired and/or use parallel computing)
end
