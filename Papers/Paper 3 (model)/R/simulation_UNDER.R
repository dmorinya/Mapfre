...
result <- foreach(k=1:600, .combine=rbind) %dopar% 
  try(expSimuML( n = 200 ,
             p = c ( 0.5 , 3 , 0.7 , 0.2 , 0.3 ) ,
             th = 3 ,
             conf = 0.95 ))
save(list="result", file="../Results/case1.RData")

# Case 2: w=0.3 and moderate underreporting (phi1=0.2; phi2=0.3) 
#result <- foreach(k=1:600, .combine=rbind) %dopar%
#  try(expSimuML (n = 200 ,
#             p = c ( 0.5 , 3 , 0.3 , 0.2 , 0.3 ) ,
#             th = 3 ,
#             conf = 0.95 ))
#save(list="result", file="../Results/case2.RData")

# Case 3: w=0.7 and severe underreporting (phi1=0.1; phi2=0.1) 
#result <- foreach(k=1:600, .combine=rbind) %dopar%
#   try(expSimuML ( n = 200 ,
#             p = c ( 0.5 , 3 , 0.7 , 0.1 , 0.1 ) ,
#             th = 3 ,
#             conf = 0.95 ))
#save(list="result", file="../Results/case3.RData")

# Case 4: w=0.3 and severe underreporting (phi1=0.1; phi2=0.1) 
#result <- foreach(k=1:600, .combine=rbind) %dopar%
#   try(expSimuML(n = 200 ,
#             p = c ( 0.5 , 3 , 0.3 , 0.1 , 0.1 ) ,
#             th = 3 ,
#             conf = 0.95 ))
#save(list="result", file="../Results/case4.RData")
