...
obstetrics <- mapfre_ts[mapfre_ts$Code_Real==31 & mapfre_ts$Code_Esp==31 & mapfre_ts$Code_Group=="10100" & mapfre_ts$Id== 1, ]
...
png("Results/obstetrics_2019_2020.png")
plot(impact)
dev.off()
summary(impact)
...
impact <- CausalImpact(bsts.model = bsts.model,
                       post.period.response = post.period.response)
png("Results/obstetrics_2019_2021.png")
plot(impact)
dev.off()
summary(impact)