...
generalm <- mapfre_ts[mapfre_ts$Code_Real==1 & mapfre_ts$Code_Esp==1 & mapfre_ts$Code_Group=="10100" & mapfre_ts$Id== 1, ]

generalm <- generalm %>% group_by(Year, WeekNum) %>% summarise(Acts=sum(Acts)) ### Remove duplicates

### La darrera setmana de 2021 té uns valors molt baixos en tots els casos, no semblen correctes
generalm <- generalm[-158, ]

generalm$Date <- totals$Date

### 2019 vs 2020
y <- generalm$Acts[year(generalm$Date)<=2020 ]
x <- totals$Total[year(totals$Date)<=2020 ]
post.period <- c(which(year(generalm$Date)==2020)[1], which(year(generalm$Date)==2020)[length(which(year(generalm$Date)==2020))])
...
png("Results/generalm_2019_2020.png")
plot(impact)
dev.off()
summary(impact)
...
impact <- CausalImpact(bsts.model = bsts.model,
                       post.period.response = post.period.response)
png("Results/generalm_2019_2021.png")
plot(impact)
dev.off()
summary(impact)