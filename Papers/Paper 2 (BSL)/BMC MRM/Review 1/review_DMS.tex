...
\begin{reply}
We agree with the reviewer that ``estimation'' is referred to parameters and ``prediction'' is usually used in relation to values of the series. We have carefully reviewed the manuscript and changed the wording accordingly.
\end{reply}
...
\textcolor{blue}{``The predicted registered values can also be obtained as $\hat{Y_t} = (1-\hat{\omega}+\hat{\omega} \cdot \hat{q}) \cdot \hat{X_t}$, and compared to the actual registered values $Y_t$. That allows computing standard forecasting error measures as Root Mean Squared Error (RMSE) or Mean Absolute Percentage Error (MAPE). Overall, the RMSE was 113,145.4 and MAPE was around 8\%, ranging between 4 to 13\% across regions. The RMSE and MAPE for each region are described in Table S1 in the Supplementary Material.''}
...
\textcolor{blue}{``XXXXXXXXXXXXXXXXXX''}
